...
def load_spreadsheet():
    ...
    header = wks.range('A1:H1')
    #header[0].value = 'besteller'
    #header[1].value = 'artikel'
    #header[2].value = 'type'
    #header[3].value = 'sku'
    header[4].value = 'Per stuk'
    header[5].value = '#'
    header[6].value = 'Totaal'
    header[7].value = '5%'
...
def add_to_spreadsheet(wks, orders):
    ...
    for user, products in orders.iteritems():
        ...
        for product in products:
            row = wks.range('A' + str(row_number) + ':H' + str(row_number))
            row[0].value, row[1].value = product['name'].split(' ', 1)
            row[2].value = product['type']
            row[3].value = "=(\"" + product['sku'] + "\")"
            row[4].value = product['price']
            row[5].value = product['qty']
            row[6].value = float(product['price']) * product['qty']
            row[7].value = math.ceil(float(row[5].value) * 0.95 * 100) / 100

            wks.update_cells(row)
            row_number += 1
        last_row = row_number - 1
        wks.update_cell(row_number, 7, '=SUM(G' + str(first_row) + ':G' + str(last_row) + ')')
        wks.update_cell(row_number, 8, '=CEILING(G' + str(row_number) + '*0.95, 0.01)')
        row_number += 2
